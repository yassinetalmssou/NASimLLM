#!/bin/bash
# One-shot submission of the thesis experiment (multi-LLM teacher comparison):
# RQ1 all teachers x small, RQ2 panel teachers x all scenarios, RQ3 primary teacher x small.
# A GPU pre-flight gate (hpc/jobs/gate.sh) runs first; every array depends on it (afterok).
...
# Environment: activate the venv (generators import nasim, needed even for --dry-run).
