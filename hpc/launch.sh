...
# ── Training hyperparameters (match EXECUTION.md template) ────────────────────
LAMBDA_DECAY=0.97
LAMBDA_MODE=competence
LAMBDA_MIN=0.05
BATCH_SIZE=64
NUM_EPOCHS=4
LLM_MIX_WEIGHT=0.5
...
mkdir -p "$OUT_ROOT/rq1"
mkdir -p "$OUT_ROOT/rq2"
mkdir -p "$OUT_ROOT/rq3"
mkdir -p logs
...
python -m nasim.scripts.run_rq1 \
  ...
  --llm-mix-weight $LLM_MIX_WEIGHT \
  --parallel

if [ -f "$OUT_ROOT/rq1/commands_${SCENARIO}.txt" ]; then
    N=$(wc -l < "$OUT_ROOT/rq1/commands_${SCENARIO}.txt")
    TOTAL_RQ1_ALL=$((TOTAL_RQ1_ALL + N))
    echo "Generated $N commands for RQ1[$SCENARIO]"
    echo "  Submit: sbatch --array=1-$N hpc/jobs/rq1.sh $OUT_ROOT/rq1/commands_${SCENARIO}.txt"
else
...
for SCENARIO in "${SCENARIOS[@]}"; do
echo "======================================================================"
echo "RQ3 [$SCENARIO]: Component Ablations"
echo "======================================================================"
...
if [ -f "$OUT_ROOT/rq3/commands_${SCENARIO}.txt" ]; then
    N=$(wc -l < "$OUT_ROOT/rq3/commands_${SCENARIO}.txt")
    TOTAL_RQ3_ALL=$((TOTAL_RQ3_ALL + N))
    echo "Generated $N commands for RQ3[$SCENARIO]"
    echo "  Submit: sbatch --array=1-$N hpc/jobs/rq3b.sh $OUT_ROOT/rq3/commands_${SCENARIO}.txt"
else
    echo "ERROR: Failed to generate RQ3[$SCENARIO] commands"
    exit 1
fi
done
...
echo "Total experiment tasks: $((TOTAL_RQ1_ALL + TOTAL_RQ2 + TOTAL_RQ3_ALL))"
echo "  - RQ1:  $TOTAL_RQ1_ALL tasks (4 conditions x 3 seeds x ${#SCENARIOS[@]} scenarios)"
echo "  - RQ2:  $TOTAL_RQ2 tasks  (2 conditions x 4 scenarios x 3 seeds)"
echo "  - RQ3:  $TOTAL_RQ3_ALL tasks (5 ablations x 3 seeds x ${#SCENARIOS[@]} scenarios)"
echo ""
echo "Command files:"
for SCENARIO in "${SCENARIOS[@]}"; do
echo "  $OUT_ROOT/rq1/commands_${SCENARIO}.txt"
echo "  $OUT_ROOT/rq3/commands_${SCENARIO}.txt"
done
echo "  $OUT_ROOT/rq2/commands.txt"
echo ""
echo "Submit commands:"
for SCENARIO in "${SCENARIOS[@]}"; do
    N1=$(wc -l < "$OUT_ROOT/rq1/commands_${SCENARIO}.txt" 2>/dev/null || echo 0)
    N3=$(wc -l < "$OUT_ROOT/rq3/commands_${SCENARIO}.txt" 2>/dev/null || echo 0)
    echo "  sbatch --array=1-$N1  hpc/jobs/rq1.sh  $OUT_ROOT/rq1/commands_${SCENARIO}.txt"
    echo "  sbatch --array=1-$N3  hpc/jobs/rq3b.sh  $OUT_ROOT/rq3/commands_${SCENARIO}.txt"
done
